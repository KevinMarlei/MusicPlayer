const objectMusic = [
   {
      gender: 'Country&Folk',
      title: 'To Weather a Storm',
      artist: 'Dan Lebowitz',
      src: 'musics/To Weather a Storm - Dan Lebowitz.mp3',
      img: 'img/country&folk.jpg'
   },
   {
      gender: 'Jazz and Blues',
      title: 'Eletrosamba',
      artist: 'Quincas Moreira',
      src: 'musics/Eletrosamba - Quincas Moreira.mp3',
      img: 'img/jazzBlues.jpg'
   },
   {
      gender: 'Country&Folk',
      title: 'Dirty Mac',
      artist: 'Endless Love',
      src: 'musics/Dirty Mac - Endless Love.mp3',
      img: 'img/country&folk.jpg'
   },
   {
      gender: 'Rock',
      title: 'Elevators Need Rock Too',
      artist: 'Spence',
      src: 'musics/Elevators Need Rock Too - Spence.mp3',
      img: 'img/rock.jpg'
   },
   {
      gender: 'My music',
      title: 'Light Infection',
      artist: 'Prague',
      src: 'musics/Light Infection.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Smells Like Team Spirit',
      artist: 'Nirvana',
      src: 'musics/Nirvana - Smells Like Teen Spirit.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Come as You Are',
      artist: 'Nirvana',
      src: 'musics/Come as You Are - Nirvana.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Chop Suey',
      artist: 'System of a Down',
      src: 'musics/System of a Down - Chop Suey (Remastered 2021).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Toxicity',
      artist: 'System of a Down',
      src: 'musics/System Of A Down - Toxicity (Official HD Video).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Kinetic',
      artist: 'DJ Sona’s',
      src: 'musics/DJ Sona’s Ultimate Skin Music Kinetic.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Legends Never Die',
      artist: 'League of Legend’s',
      src: 'musics/Legends Never Die (ft. Against The Current) [OFFICIAL AUDIO]  Worlds 2017 - League of Legends.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Gravity Wall',
      artist: 'RE: Creator’s',
      src: 'musics/Gravity Wall.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Fly Me To The Moon',
      artist: 'Frank Sinatra',
      src: 'musics/Fly Me To The Moon (2008 Remastered).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'A Cruel Angel’s Thesi’s',
      artist: 'Frank Sinatra',
      src: 'musics/Shiro Sagisu, Yoko Takahashi - A Cruel Angels Thesis - (Evangelion Original Soundtrack).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Tremor',
      artist: 'Dimitri Vegas, Martin Garrix, Like Mike',
      src: 'musics/Dimitri Vegas, Martin Garrix, Like Mike - Tremor (Official Music Video).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Innocence',
      artist: 'NoisyCell',
      src: 'musics/NoisyCell - Innocence ［OFFICIAL MUSIC VIDEO］.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Anima Rossa',
      artist: 'PornoGraffitti',
      src: 'musics/Animarossa.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Tomorrow',
      artist: 'Marina Ismail',
      src: 'musics/Tomorrow by Tsunematsu  Ayumi (Marina Ismail).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Ciclo sem Fim',
      artist: 'O Rei Leão',
      src: 'musics/O Rei Leão - Ciclo sem Fim.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Pra Você',
      artist: 'Onze:20',
      src: 'musics/Onze20 - Pra Você (Áudio).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'No Rancho Fundo',
      artist: 'Xitãozinho e Xororó',
      src: 'musics/Chitãozinho e Xororó   No Rancho Fundo.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Fio de Cabelo',
      artist: 'Xitãozinho e Xororó',
      src: 'musics/Chitãozinho e Xororó - Fio De Cabelo.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Creep',
      artist: 'RadioHead',
      src: 'musics/Creep.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Stay With Me',
      artist: 'Matsubara Miki',
      src: 'musics/Miki Matsubara - Stay With Me HD (Club Mix).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Otherside',
      artist: 'Red Hot Chili Pepper’s',
      src: 'musics/Red Hot Chili Peppers - Otherside (Lyrics).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Cheia de Manias',
      artist: 'Raça Negra',
      src: 'musics/Raça Negra - Cheia de Manias.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Super Shy',
      artist: 'New jeans',
      src: 'musics/NewJeans - Super Shy.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'How Sweet',
      artist: 'New jeans',
      src: 'musics/NewJeans (뉴진스) How Sweet Official MV.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Under Star',
      artist: 'Hajime no Ippo',
      src: 'musics/Hajime No Ippo OP 1 - Under Star - Shocking Lemon (full version).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Opening 2',
      artist: 'Hajime no Ippo',
      src: 'musics/Hajime No Ippo OP 2 - Inner Light - Shocking Lemon (full version).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Opening 3',
      artist: 'Hajime no Ippo',
      src: 'musics/Opening 3 de Hajime no Ippo full.mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Tanjirou no Uta',
      artist: 'Kimetsu No Yaiba',
      src: 'musics/「Kamado Tanjiro no Uta」Demon Slayer Kimetsu no Yaiba (鬼滅の刃) [Demon Slayer ep 19 Ending Song].mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Past Lives',
      artist: 'SapientDream',
      src: 'musics/sapientdream - Past Lives (Lyrics).mp3',
      img: 'img/cerejeiras.gif'
   },
   {
      gender: 'My music',
      title: 'Sorisso Resplandecente',
      artist: 'Dragon ball GT',
      src: 'musics/DBGT - Sorriso Resplandecente ☁️.mp3',
      img: 'img/cerejeiras.gif'
   },
]